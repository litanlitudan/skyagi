<script lang="ts">
	...
	onMount(() => {
		socket = new WebSocket('wss://langchain-558b0f2c14.wolf.jina.ai/runskyagi');
			...
			if (event.data.includes('exiting')) {
				...
				if (isReponseType) {
					if (response.result && response.result != ': ' && response.result != ':')
						messageStore.set(`${response.result}?role=assistant`);
				} else if (isHumanPromptType) {
					// do nothing, as the UI will trigger a message sent
				} else {
					messageStore.set(`Unknown message: ${event.data}?role=assistant`);
				}
			}
		});
		...
		socket.addEventListener('error', function (event) {
			if (event)
				messageStore.set(
					`ws connection closed with exception ${JSON.stringify(event)}?role=assistant`
				);
		});
	...
	function onSendMessage() {
		if (message.length > 0) {
			messageStore.set(`${message}?role=you`);
			sendMessage(message);
			message = '';
		}
	}
